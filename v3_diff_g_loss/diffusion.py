...
class DiffusionBridge(L.LightningModule):
    ...
    def make_lambda_map(
        ...
        """
        Build lambda_hat guidance map for generator.

        This is used only as generator guidance.

        Important:
        - Do NOT use real x0 here.
        - Use current recursive estimate x_r.
        - This keeps training and inference consistent.

        Args:
            t: [B]
            y: source image, [B, C, H, W]
            x_r: current recursive estimate, [B, C, H, W]

        Returns:
            lambda_hat: [B, C, H, W]
        """
        if x_r is None:
            x_r = torch.zeros_like(y)
        ...
        # lambda_hat = AdaptiveBridgeLambda(t, x_r, y)
        # Use x_r instead of ground-truth x0.
        lambda_hat = self._get_lambda(t, x_r_ref, y_ref)
    ...
    def sample_x0(self, y, generator):
        ...
        for t in timesteps:
            ...
            for _ in range(self.n_recursions):
                ...
                lambda_map = self.make_lambda_map(
                    t=t,
                    y=y,
                    x_r=x0_r,
                    detach=True,
