...
class BridgeRunner(L.LightningModule):
    ...
    def training_step(self, batch):
        ...
        for _ in range(self.n_recursions):
            x0_r = self.generator(torch.cat((x_t, y), axis=1), t, x_r=x0_r)
        x0_pred = x0_r

        # Posterior sampling q(x_{t-1} | x_t, y, x0_pred)
        x_tm1_pred = self.diffusion.q_posterior(t, x_t, x0_pred, y).detach()

        # Perform fake data prediction
        disc_out = self.discriminator(x_tm1_pred, x_t.detach(), t)
        ...
        # Get x_t via forward process. detach=False is required so the
        # adaptive bridge field can be optimized through the generator loss.
        x_t = self.diffusion.q_sample(t, x0, y, detach=False)
        ...
        g_loss = self.lambda_rec_loss * rec_loss + adv_loss + lambda_reg
    ...
    def configure_optimizers(self):
        # The adaptive bridge field in self.diffusion is learnable and is
        # optimized jointly with the generator using the original SelfRDB losses.
        optimizer_g = Adam(
            list(self.generator.parameters()) + list(self.diffusion.parameters()),
            lr=self.lr_g,
